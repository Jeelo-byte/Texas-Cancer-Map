...
const template = JSON.parse(
  fs.readFileSync("./public/Texas_County_Boundaries.json", "utf8"),
);
...
const filteredLines = [
  header,
  ...lines.slice(1).filter((line) => line.split(",").length === numColumns),
];
...
countyRecords.forEach((row) => {
  ...
  if (county) {
    ...
    county.AgeAdjustedDeathRate =
      parseFloat(
        row["Age-Adjusted Incidence Rate([rate note]) - cases per 100,000"],
      ) || 0;
    county.AverageAnnualDeaths = parseFloat(row["Average Annual Count"]) || 0;
    county.RecentTrend_PercentPerYear =
      parseFloat(
        row["Recent 5-Year Trend ([trend note]) in Incidence Rates"],
      ) || 0;
    matched++;
  }
});
...
console.log(
  `Merged data for ${matched} counties. Output written to public/data.json`,
);
...
const sample = template.find(
  (c) =>
    c.AgeAdjustedDeathRate ||
    c.AverageAnnualDeaths ||
    c.RecentTrend_PercentPerYear,
);
if (sample) {
